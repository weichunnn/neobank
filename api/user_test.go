	...
	testCases := []struct {
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				// gomock.eq cannot work since password hash are diff each time due to salt
				store.EXPECT().
		...
		},
		{
			...
			body: gin.H{
				"username":  "invalidCharUsername-#1",
				"password":  password,
				"full_name": user.FullName,
				"email":     user.Email,
			},
...
func requireBodyMatchUser(t *testing.T, body *bytes.Buffer, user db.User) {
	...
	require.Empty(t, gotUser.HashedPassword) // not returned
}
