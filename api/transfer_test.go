...
func TestTransferAPI(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "FromAccountNotExist",
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().GetAccount(gomock.Any(), gomock.Eq(account1.ID)).Return(db.Account{}, sql.ErrNoRows)
		...
		{name: "GetAccountValidationError",
			body: gin.H{
				"from_account_id": account1.ID,
				"to_account_id":   account2.ID,
				"amount":          amount,
